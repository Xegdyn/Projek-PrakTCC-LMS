document.addEventListener("DOMContentLoaded", () => {
    ...
    const getPengunjung = async () => {
      try {
        const response = await axios.get("http://localhost:3200/pengunjung");
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        if (id) {
          await axios.put(`http://localhost:3200/pengunjung/${id}`, { nama, kode_induk: kodeInduk });
        } else {
          await axios.post("http://localhost:3200/pengunjung", { nama, kode_induk: kodeInduk });
        }
        form.reset();
    ...
    window.editPengunjung = async (id) => {
      try {
        const response = await axios.get(`http://localhost:3200/pengunjung/${id}`);
    ...
    window.deletePengunjung = async (id) => {
      try {
        await axios.delete(`http://localhost:3200/pengunjung/${id}`);
        getPengunjung();
      } catch (error) {
